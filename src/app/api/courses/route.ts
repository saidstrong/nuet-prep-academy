...
export async function GET() {
  try {
    console.log('🔍 Courses API called - attempting hybrid approach');
    
    // Check cache first
    const cachedCourses = cache.get(CACHE_KEYS.COURSES);
    if (cachedCourses) {
    ...
    try {
      ...
      // Test if we can query courses with a simple query first
    ...
    // Cache the result
    cache.set(CACHE_KEYS.COURSES, processedCourses, CACHE_TTL.COURSES);
